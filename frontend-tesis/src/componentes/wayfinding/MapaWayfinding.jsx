...
const MapaWayfinding = ({ origen, destino, ubicacionUsuario, onRutaCalculada, isNavigating = false, heading = null }) => {
  ...
  // ── Compartir ubicación seleccionada ──────────────────────────────────────
  const handleCompartir = async (ubicacion) => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Campus U. de Talca', text: texto, url });
      } catch (err) {
          ...
          return (
            <div className="info-card-flotante">
              ...
              <div className="info-card-actions">
                <button className="info-card-share" onClick={() => handleCompartir(ubicacionSeleccionada)}>
